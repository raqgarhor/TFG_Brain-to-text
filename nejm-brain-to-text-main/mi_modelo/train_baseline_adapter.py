...
def build_model(model_args, adapter_bottleneck, adapter_dropout, logit_adapter=False):
...
def load_pretrained_baseline(model, checkpoint_path):
    ...
    missing = [
        ...
        if key.startswith("adapter.") or key.startswith("logit_adapter.")
    ]
    other_missing = [
        key
        for key in result.missing_keys
        if not key.startswith("adapter.") and not key.startswith("logit_adapter.")
    ]
    if other_missing or result.unexpected_keys:
        raise RuntimeError(
            f"Carga inesperada. missing={other_missing}, unexpected={result.unexpected_keys}"
        )
    if missing:
        print("Pesos nuevos inicializados:", ", ".join(missing))
    return checkpoint
...
def set_trainable(model, mode):
    ...
    if hasattr(model, "logit_adapter"):
        for param in model.logit_adapter.parameters():
            param.requires_grad = True

    if mode in {"adapter_output", "adapter_day_output"}:
        for param in model.out.parameters():
            param.requires_grad = True

    if mode == "adapter_day_output":
...
def main():
    parser = argparse.ArgumentParser(description="Entrena baseline preentrenado con adaptador residual.")
    ...
    parser.add_argument("--logit_adapter", action="store_true", help="Anadir corrector temporal residual sobre logits.")
    parser.add_argument("--trainable", choices=["adapter", "adapter_output", "adapter_day_output", "all"], default="adapter_day_output")
    parser.add_argument("--output_dir", default=str(THIS_DIR / "salidas" / "baseline_adapter"))
    ...
    train_dataset = HDF5TrialDataset(args.data_dir, sessions, "train", session_to_day, args.max_train_trials)
    val_dataset = HDF5TrialDataset(args.data_dir, sessions, "val", session_to_day, args.max_val_trials)
    ...
    model = build_model(
        ...
        logit_adapter=args.logit_adapter,
    ).to(device)
    checkpoint = load_pretrained_baseline(model, args.checkpoint)
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max(args.num_batches, 1), eta_min=args.lr_min)
    ctc_loss = torch.nn.CTCLoss(blank=0, reduction="mean", zero_infinity=True)

    print("Baseline preentrenado + adaptador residual")
    ...
    print(f"Corrector temporal de logits: {'si' if args.logit_adapter else 'no'}")
    print(f"PER inicial en esta validacion: {initial_per:.4f}")
    ...
    for step in range(1, args.num_batches + 1):
        ...
        loss = ctc_loss(logits.log_softmax(2).permute(1, 0, 2), labels, input_lens, seq_lens)
        ...
        if step == args.num_batches or step % args.eval_every == 0:
            ...
            print(f"  PER validacion adapter: {per:.4f}")
            if per < best_per:
                best_per = per
    ...
    print(f"Mejor PER adapter: {best_per:.4f}")
